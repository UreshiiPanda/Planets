...
unsigned int	MarsTex, VenusTex, EarthTex, JupiterTex, SaturnTex, UranusTex, NeptuneTex;		// texture object
int	NowPlanet;
int NowTex;
...
{
        { "Venus",      "venus.bmp",     0.95f, VenusDL, 'v', MarsTex },
        { "Earth",      "earth.bmp",     1.00f, EarthDL, 'e', VenusTex },
        { "Mars",       "mars.bmp",      0.53f, MarsDL, 'm', EarthTex },
...
Display( )
{
	...
	if( NowTex == REPLACE or NowTex == MODULATE )
    ...
    float xc = 0.;
    float zc = 0.;
    float xli = xc + cosf(2*F_PI*Time) * 3 * Planets[NowPlanet].scale;
    float yli = 0.;
    float zli = zc + sin(2*F_PI*Time) * 3 * Planets[NowPlanet].scale;

    if ( LightType == POINT )
        SetPointLight(GL_LIGHT0, xli, yli, zli, Colors[LightColor][0], Colors[LightColor][1], Colors[LightColor][2]);
    else
        SetSpotLight(GL_LIGHT0, xli, yli, zli, 0, 0, 0, Colors[LightColor][0], Colors[LightColor][1], Colors[LightColor][2]);
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
        ...
        case 'T':
            {
                if (NowTex == NOTEX) NowTex = REPLACE;
                else if (NowTex == REPLACE) NowTex = MODULATE;
                else NowTex = NOTEX;
            }
            break;
